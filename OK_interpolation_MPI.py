...
def main(size_grid,samplepoints,xyz_points,neighbour):
    ...
    for grid in size_grid:
        if grid not in xyz_points:
            result_grids.append(OKHelper.OKkriging01(grid,neighbour,samplepoints)) 
            end2=time.clock()
...
if __name__ == '__main__':
    ...
    samplepoints,xyz_points=OKHelper.readsamplepoints('S14_8_train.txt',1,1,1) # read knownpoints
    save_txt_path="OK_Interpolation_result"+str(neighbour)+"_"+area+".txt" #save result path
    save_vtk_path="OK_Interpolation_result"+str(neighbour)+"_"+area+".vtk" #save 3D result
    if  comm_rank==0:
        start2 =time.clock()
        size_grid=OKHelper.new3Dgrid(30)# new empty 3D grid 30*30*30
